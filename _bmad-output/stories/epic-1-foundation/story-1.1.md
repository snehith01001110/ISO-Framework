...
## Description
Initialize the three-crate Cargo workspace defined in PRD Section 3: `worktree-core` (library), `worktree-core-cli` (binary `wt`), and `worktree-core-mcp` (binary MCP server). Set up GitHub Actions CI for macOS and Linux runners, enforce `cargo clippy -D warnings` as a gate, and create skeleton `Cargo.toml` files with the dependency list from PRD Section 14. Feature flags for optional platform modules and future `gix`/`git2` conflict detection must be declared but empty.
...
- [ ] `worktree-core/Cargo.toml` lists all 14 dependencies from PRD Section 14 with exact version constraints
- [ ] Feature flags `conflict-detection` (for future `gix`/`git2`) and `windows` (platform module) exist but are empty
- [ ] `worktree-core-cli` and `worktree-core-mcp` depend on `worktree-core` as a path dependency
- [ ] Rust MSRV set to 1.75 in all `Cargo.toml` files via `rust-version` field
- [ ] `.gitignore` excludes `target/` and platform artifacts

## Tasks
- [ ] Create repo root `Cargo.toml` with `[workspace]` containing members `["worktree-core", "worktree-core-cli", "worktree-core-mcp"]`
- [ ] Create `worktree-core/Cargo.toml` with all dependencies: `fd-lock = "4"`, `sysinfo = "0.37"`, `uuid = { version = "1", features = ["v4"] }`, `reflink-copy = "0.1"`, `junction = "1"`, `jwalk = "0.8"`, `filesize = "0.2"`, `directories = "6"`, `dunce = "1"`, `thiserror = "2"`, `serde = { version = "1", features = ["derive"] }`, `serde_json = "1"`, `chrono = { version = "0.4", features = ["serde"] }`, `rand = "0.8"`, `sha2 = "0.10"`
- [ ] Create `worktree-core/src/lib.rs` with placeholder module declarations matching PRD Section 3 file layout
...
- [ ] Create `worktree-core-cli/Cargo.toml` and `worktree-core-cli/src/main.rs` with minimal `fn main()`
- [ ] Create `worktree-core-mcp/Cargo.toml` and `worktree-core-mcp/src/main.rs` with minimal `fn main()`
...
- [ ] Declare feature flags in `worktree-core/Cargo.toml`: `conflict-detection = []`, `windows = []`
- [ ] Set `rust-version = "1.75"` in all three `Cargo.toml` files
- [ ] Verify `cargo build --workspace` succeeds locally

## Technical Notes
- Crate structure matches PRD Section 3 exactly: `worktree-core/`, `worktree-core-cli/`, `worktree-core-mcp/`
