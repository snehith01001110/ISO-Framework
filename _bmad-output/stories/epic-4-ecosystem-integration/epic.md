...
## Summary
Deliver ecosystem-specific adapters (pnpm, uv, Cargo), replace the CLI-based conflict detection path with `gix::Repository::merge_trees()` behind a feature flag, provide language bindings for Node.js (napi-rs) and Python (PyO3, stretch goal), implement worktree pooling for instant checkout, validate external integration by having at least one third-party project (Claude Squad or workmux) consume `worktree-core` as a library, and publish a documentation site with mdBook.
...
- napi-rs Node.js binding generating TypeScript types; published to npm as `@worktree-core/node`; tested on Node 18+.
- PyO3 Python binding (stretch goal) installable via `pip install worktree-core`; tested on Python 3.9+.
...
- At least one external project (Claude Squad or workmux) consuming `worktree-core` as a library dependency.
