...
## User Story
As a maintainer of iso-code, I want at least one external project (Claude Squad or workmux) consuming iso-code as a library dependency so that I can validate the API design with real-world usage and demonstrate ecosystem value.

## Description
Integrate iso-code into at least one external project as a library dependency. The primary target is Claude Squad (Go, using iso-code via CLI or MCP) or workmux (Rust, using iso-code as a crate dependency). This validates that the public API is ergonomic, the error types are actionable, and the library solves real problems that these tools have documented (e.g., `claude-squad#260` for environment setup, workmux port collisions). The integration should be contributed upstream as a PR.

## Acceptance Criteria
- [ ] At least one external project has a working integration with iso-code
...
- [ ] Integration uses iso-code's public API correctly (no private API access)
- [ ] Integration tests pass in the external project's CI
- [ ] README or documentation in the external project references iso-code
...
- [ ] Implement integration: replace ad-hoc worktree management with iso-code calls
- [ ] Add iso-code as a dependency in the external project's build system
- [ ] Write integration tests that exercise the iso-code API in the external project's context
...
## Technical Notes
- PRD Section 15 M3 ship criterion: "At least one external project consuming iso-code as a library dependency."
...
## Test Hints
- For workmux: replace workmux's internal worktree creation with `iso_code::Manager::create()`, run workmux's test suite
- For Claude Squad: configure `wt hook` as the WorktreeCreate hook, verify worktrees are created correctly
- Verify the integration solves a documented bug: create a scenario that triggers the original bug, verify iso-code prevents it
